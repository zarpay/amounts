...
RSpec.describe "Amount RSpec matchers" do
  describe "eq_amount" do
    ...
    it "matches another amount instance" do
      expect(Amount.of_usdc("1.50")).to eq_amount(Amount.of_usdc("1.50"))
    end

    it "fails with a helpful message" do
      expect do
        expect(Amount.of_usdc("1.50")).to eq_amount("USDC|2.00")
  ...
  describe "predicate matchers" do
    it "matches zero amounts" do
      expect(Amount.of_usdc(0, from: :atomic)).to be_zero_amount
    end
    ...
    it "matches negative amounts" do
      expect(Amount.of_usdc("-1.50")).to be_negative_amount
    end
  end
  ...
  describe "be_approximately_amount" do
    ...
    it "matches within an amount delta" do
      expect(Amount.of_usdc("1.55")).to be_approximately_amount(Amount.of_usdc("1.50"), within: Amount.of_usdc("0.10"))
    end

    it "fails with a helpful message" do
      expect do
        expect(Amount.of_usdc("1.80")).to be_approximately_amount(:USDC, "1.50", within: "0.10")
